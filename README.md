...
```text
首次安装并启动：one-click-start.command（仅一次）
日常命令行启动（推荐）：./.venv_paddle/bin/python -m backend.server
带 GUI 窗口启动：start-backend-pipeline-gui.command（可选）
```
...
普通用户优先双击 `.cmd` / `.command` 文件。如果你需要手动调试，可以用下面的命令。
...
本项目提供两套日志系统，分别记录后端识别情况和浏览器端操作事件。

### 后端日志（Python）

启动后端后，所有 `print()` 输出自动写入 `logs/` 目录。每个进程（server、YOLO worker、OCR worker）独立写日志，文件按时间戳命名。

```bash
# 实时查看后端日志
tail -f logs/backend-main-*.log

# 只看验证码识别记录
grep '\[captcha\]' logs/backend-main-*.log
```

日志会自动轮转，保留最近 5 个文件，每个不超过 2 MB。

### 浏览器端事件日志（油猴脚本）

油猴脚本内置事件日志 ring buffer（最近 200 条），记录每次 API 返回、弹窗检测和自动关闭动作，持久化在 Tampermonkey 存储中。

**无需打开 DevTools Console 即可查看：**

- 在 GLM Coding 页面上按 **`F7`** 弹出日志窗口，可复制、清空
- 再按一次 `F7` 关闭窗口

**在 DevTools Console 中（如无调试暂停冲突）：**
...
```
[2026-06-24T01:56:20.000Z] preview_ok {"bizId":"xxx","payAmount":298}
[2026-06-24T01:56:22.000Z] popup_detected {"type":"valid_payment","bizId":"xxx"}
[2026-06-24T01:56:25.000Z] popup_closed {"type":"rate_limit","count":3}
```
